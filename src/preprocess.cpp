...
void Preprocess::s10u_handler(const sensor_msgs::PointCloud2::ConstPtr &msg) {
  ...
  if (feature_enabled) {
    ...
    for (uint i = 0; i < plsize; i++) {
      ...
      added_pt.curvature =
          (pl_orig.points[i].timestamp * 1e-6 - time_head) * 1e3; // ms
      if (pl_orig.points[i].row_pos < N_SCANS) {
        pl_buff[pl_orig.points[i].row_pos].push_back(added_pt);
      }
    }
